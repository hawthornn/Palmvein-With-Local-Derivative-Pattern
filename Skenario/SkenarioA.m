%% Source Code Tugas Akhir Skenario A (Akhmad Faizal Akbar 1103110183)
% Dataset CASIA Menggunakan 850nm tangan kiri

%% Deskripsi Skenario A
% Order = 2 dan Scale = 1 sampai 6
% Plam(Telapak Tangan)
% Perblock(16 block)
%% Inisialisasi
clc;
    ...
    for j=1:nPalm
        ...
        for ii=1:rrange:resize(1,2)
            for jj=1:crange:resize(1,1)
                feature{r,c} = LDPsc(vein(ii:rrange+ii-1,jj:crange+jj-1),0,1);
                feature{r,c+1} = LDPsc(vein(ii:rrange+ii-1,jj:crange+jj-1),45,1);
                feature{r,c+2} = LDPsc(vein(ii:rrange+ii-1,jj:crange+jj-1),90,1);
                feature{r,c+3} = LDPsc(vein(ii:rrange+ii-1,jj:crange+jj-1),135,1);
                c = c + 4;
            end
        end
        c = 1;
